/*
 * Copyright 2009 Google Inc. Licensed under the Apache License, Version 2.0 (the "License"); you
...
/**
 * This file is derived from original HandlerManager. It allows to "copy" an handler manager (use
 * the same bus and resink the events)
 */

/**
 * Manager responsible for adding handlers to event sources and firing those handlers on passed in
 * events. Primitive ancestor of {@link EventBus}, and used at the core of
...
public class HandlerManager implements HasHandlers {
	...
	private int eventsToSink = 0;

	/**
	 * Creates a handler manager with a source to be set on all events fired via
	 * {@link #fireEvent(GwtEvent)}. Handlers will be fired in the order that they are added.
	 *
	...
	public HandlerManager(HandlerManager handlerManager, Object source) {
		if (handlerManager != null) {
			this.eventBus = handlerManager.eventBus;
			this.eventsToSink = handlerManager.eventsToSink;
		} else {
			this.eventBus = new Bus(false);
		}
		this.source = source;
	}
	 ...
	 * <p>
	 * Any exceptions thrown by handlers will be bundled into a {@link UmbrellaException} and then
	 ...
	 * <p>
	 * Note, any subclass should be very careful about overriding this method, as adds/removes of
	 * handlers will not be safe except within this implementation.
	 *
